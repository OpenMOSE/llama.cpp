// lod_attn_kernels.cu - standalone PyTorch extension packaging the LoD attention
// decode kernels EXTRACTED VERBATIM from llama.cpp ggml/src/ggml-cuda/lod-attn.cu
// (branch `lod`). The kernel region below is byte-identical to the ggml tree so
// results match the llama.cpp fused op bit-for-bit given identical inputs.
//
// Build: loaded by lod_attn_ext.py via torch.utils.cpp_extension (CUDA or ROCm -
// PyTorch hipifies this file automatically on ROCm builds).
//
// Exposed op (in-op selection path, mirrors GGML_OP_LOD_ATTN with sel == NULL):
//   lod_attn_forward(q, K, V, Ks, Vs, prev_end, P_rt, scale, n_top, sel_head, ps)
//     q     [nq, Hq, Dk]  f32, contiguous
//     K, V  [T, Hkv, D]   f16 or f32, contiguous (rows past prev_end+nq unread)
//     Ks,Vs [P_cap,Hkv,D] f32, contiguous - MUTATED: the op folds the current
//                          nq tokens into their page rows (ggml semantics)
//     returns out [nq, Hq, Dv] f32
//
// NOT included here: the lds/mega ablation variants' host dispatch (kernels are
// present but unused), the FA prefill path (graph-level, see the spec doc).
...
#define HPT_CHECK(x) TORCH_CHECK(x, #x)

// portability shim (the ONLY deviation from the verbatim ggml source): ROCm's
// warp-sync builtins demand a 64-bit mask on wave64 targets under the flags the
// torch extension builder uses; numerics are identical.
#if defined(__HIP_PLATFORM_AMD__)
#define LOD_SHFL_XOR(x, off, w) __shfl_xor_sync(0xffffffffffffffffull, (x), (off), (w))
#else
#define LOD_SHFL_XOR(x, off, w) __shfl_xor_sync(0xffffffffu, (x), (off), (w))
#endif

// ============================ BEGIN verbatim kernel region (from lod-attn.cu)
...
#pragma unroll
        for (int off = tile_sz/2; off > 0; off >>= 1) {
            for (int j = 0; j < HPT; ++j) {
                sj[j] += LOD_SHFL_XOR(sj[j], off, tile_sz);
            }
        }
...
static __global__ void lod_attn_split_lds_f(
    ...
    for (int c = spl; c < n_cols; c += LOD_ATTN_SPLITS_LDS) {
        ...
        for (int d = threadIdx.x; d < Dk; d += blockDim.x) {
            kbuf[d] = kf16 ? __half2float(((const half *) kp)[d]) : ((const float *) kp)[d];
        }
        for (int d = threadIdx.x; d < Dv; d += blockDim.x) {
            vbuf[d] = vf16 ? __half2float(((const half *) vp)[d]) : ((const float *) vp)[d];
        }
        __syncthreads();
...
// ============================ END verbatim kernel region

static inline int64_t bstride(const torch::Tensor & t, int dim) {
    return t.stride(dim)*t.element_size();
}

torch::Tensor lod_attn_forward(
        torch::Tensor q, torch::Tensor K, torch::Tensor V,
        torch::Tensor Ks, torch::Tensor Vs,
        int64_t prev_end, int64_t P_rt, double scale, int64_t n_top,
        bool sel_head, int64_t page_size) {
    TORCH_CHECK(q.is_cuda() && q.dtype() == torch::kFloat32 && q.is_contiguous());
    TORCH_CHECK(K.is_contiguous() && V.is_contiguous() && Ks.is_contiguous() && Vs.is_contiguous());

    const int nq  = q.size(0);
    const int Hq  = q.size(1);
    const int Dk  = q.size(2);
    const int Hkv = K.size(1);
    const int Dv  = V.size(2);
    const int P   = Ks.size(0);   // capacity pages (>= P_rt)
    const int ps  = (int) page_size;
    const int g   = Hq/Hkv;

    TORCH_CHECK(Dk <= LOD_ATTN_MAX_D && Dv <= LOD_ATTN_MAX_D && Dk%2 == 0 && Dv%2 == 0);
    TORCH_CHECK(g%HPT == 0 && g <= 32 && P <= LOD_ATTN_MAX_P && nq <= 8);
    TORCH_CHECK(Dk%64 == 0, "in-op selection: lanes split the head dim evenly");

    auto stream = at::cuda::getCurrentCUDAStream();
    const int warp_size = at::cuda::getCurrentDeviceProperties()->warpSize;

    const int k_f16 = K.dtype() == torch::kFloat16;
    const int v_f16 = V.dtype() == torch::kFloat16;
    ...
    auto st  = torch::tensor({(int) prev_end, (int) P_rt}, opts_i);
    auto ws_scores = torch::empty({(int64_t) Hkv*P}, opts_f);
    const int n_sets = sel_head ? Hkv : 1;
    auto ws_sel = torch::empty({(int64_t) n_sets*n_top}, opts_i);

    const int n_rows = nq*Hq;
    auto part = torch::empty({(int64_t) n_rows*LOD_ATTN_SPLITS*(2 + Dv)}, opts_f);
    auto out  = torch::empty({nq, Hq, Dv}, opts_f);

    const uint64_t q_nb1 = bstride(q, 0),  q_nb2 = bstride(q, 1);
    const uint64_t k_nb1 = bstride(K, 0),  k_nb2 = bstride(K, 1);
    const uint64_t v_nb1 = bstride(V, 0),  v_nb2 = bstride(V, 1);
    const uint64_t ks_nb1 = bstride(Ks, 0), ks_nb2 = bstride(Ks, 1);
    const uint64_t vs_nb1 = bstride(Vs, 0), vs_nb2 = bstride(Vs, 1);
    ...
    const auto launch = [&](auto tile_tag) {
        ...
        lod_attn_score_fold_f<TS><<<P*Hkv + Hkv, TS, 0, stream>>>(
                (const char *) q.data_ptr(), (const char *) Ks.data_ptr(),
                (const char *) K.data_ptr(), (const char *) V.data_ptr(),
                (float *) Ks.data_ptr(), (float *) Vs.data_ptr(), st.data_ptr<int>(),
                ws_scores.data_ptr<float>(),
                Dk, Dv, nq, Hq, Hkv, P, ps,
                q_nb1, q_nb2, ks_nb1, ks_nb2, vs_nb1, vs_nb2,
                k_nb1, k_nb2, v_nb1, v_nb2, k_f16, v_f16);

        if (P <= LOD_TOPK_SORT_MAX) {
            lod_attn_topk_sort<<<n_sets, 256, 0, stream>>>(
                    ws_scores.data_ptr<float>(), ws_sel.data_ptr<int>(), st.data_ptr<int>(),
                    (int) n_top, P, Hkv, sel_head ? 1 : 0);
        } else {
            lod_attn_topk<<<n_sets, 256, 0, stream>>>(
                    ws_scores.data_ptr<float>(), ws_sel.data_ptr<int>(), st.data_ptr<int>(),
                    (int) n_top, P, Hkv, sel_head ? 1 : 0);
        }
        ...
        lod_attn_split_f<TS><<<grid, block, 0, stream>>>(
                (const char *) q.data_ptr(), (const char *) K.data_ptr(), (const char *) V.data_ptr(),
                (const char *) Ks.data_ptr(), (const char *) Vs.data_ptr(),
                ws_sel.data_ptr<int>(), st.data_ptr<int>(),
                part.data_ptr<float>(),
                Dk, Dv, Hq, Hkv, P, (int) n_top, ps,
                q_nb1, q_nb2, k_nb1, k_nb2, v_nb1, v_nb2,
                ks_nb1, ks_nb2, vs_nb1, vs_nb2,
                (float) scale, logn, k_f16, v_f16, sel_head ? 1 : 0);

        const dim3 grid_m(n_rows, (Dv + LOD_ATTN_MERGE_SLICE - 1)/LOD_ATTN_MERGE_SLICE, 1);
        lod_attn_merge_f<LOD_ATTN_SPLITS><<<grid_m, LOD_ATTN_MERGE_SLICE, 0, stream>>>(
                part.data_ptr<float>(), (float *) out.data_ptr(), Dv, Hq);
    };
    if (warp_size == 64) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("lod_attn_forward", &lod_attn_forward,
          "LoD attention fused decode read (in-op selection, mutates page sums)");
}
