...
class GGUFWriter:
    ...
    def add_head_count_kv_recurrent(self, count: int | Sequence[int]) -> None:
        if isinstance(count, int):
            self.add_uint32(Keys.Attention.HEAD_COUNT_KV_RECURRENT.format(arch=self.arch), count)
        else:
            self.add_array(Keys.Attention.HEAD_COUNT_KV_RECURRENT.format(arch=self.arch), count)
