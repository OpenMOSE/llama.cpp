...
#define RWKV7_DOUBLE_EXP_CLAMP 20.0f
static __device__ __forceinline__ float rwkv7_w_double_exp(float x) {
    float inner = __expf(fminf(x, RWKV7_DOUBLE_EXP_CLAMP));
    return __expf(-inner);
}


template <int block_size>
//Added Exp with clamp
static __global__ void rwkv_wkv7_f32(
...
#ifndef RWKV7_DOUBLE_EXP_ON_W
#define RWKV7_DOUBLE_EXP_ON_W 1
#endif
    ...
    for (int t = batch_i * n_seq_tokens * C + head_i * head_size + tid;
        ...
        __syncthreads();
        _r[tid] = r[t];
#if RWKV7_DOUBLE_EXP_ON_W
        _w[tid] = rwkv7_w_double_exp(w[t]);   
#else
        _w[tid] = w[t];
#endif
        _k[tid] = k[t];
        ...
        #pragma unroll
        for (int j = 0; j < head_size; j += 4) {
            const float4 &aa = (const float4&)_a[j];
            const float4 &ss = (const float4&)state[j];
            sa += aa.x * ss.x;
            sa += aa.y * ss.y;
            sa += aa.z * ss.z;
            sa += aa.w * ss.w;
        ...
        #pragma unroll
        for (int j = 0; j < head_size; j += 4) {
            const float4 &rr = (const float4&)_r[j];
            const float4 &ww = (const float4&)_w[j];
            const float4 &kk = (const float4&)_k[j];
            const float4 &bb = (const float4&)_b[j];
                  float4 &ss = (      float4&)state[j];
            ...
            ss.x = ss.x * ww.x + kv.x + sa * bb.x;
            ss.y = ss.y * ww.y + kv.y + sa * bb.y;
            ss.z = ss.z * ww.z + kv.z + sa * bb.z;
            ss.w = ss.w * ww.w + kv.w + sa * bb.w;
